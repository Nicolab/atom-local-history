...
Purge.prototype.onError = function onError(err, revFilePath) {
  ...
  var message = '- Can not purge the local history: <em>' + localHistoryPath + '</em>';
...
Purge.prototype.purgeRevFiles = function purgeRevFiles(files) {
  ...
  var _this       = this;
  var daysLimit   = atom.config.get('local-history.daysLimit');
  var lastRemoved = 0;

  for(var key in files) {

    var fileDays = files[key].days;
    var days     = Object.keys(fileDays);
